...
my $obj = RuntimeConflictClass->new( runtime_a => 'init_value_a', runtime_b => 'init_value_b' );
...
dies_ok { $obj2->runtime_a('runtime_value_a') } 'Cannot set runtime_a when runtime_b is already set';

# Test setting runtime_a first works
my $obj3 = RuntimeConflictClass->new();
lives_ok { $obj3->runtime_a('runtime_value_a') } 'Can set runtime_a when runtime_b is not set';
is( $obj3->runtime_a, 'runtime_value_a', 'runtime_a value is correct' );

lives_ok { $obj3->runtime_b('runtime_value_b') } 'Can set runtime_b when runtime_a is set (conflicts are not bidirectional)';
...
my $multi_obj = MultiRuntimeConflictClass->new( primary => 'primary_value', alt1 => 'alt1_value', alt2 => 'alt2_value' );
...
dies_ok { $multi_obj2->primary('primary_runtime') } 'Cannot set primary when alt1 is already set';

my $multi_obj3 = MultiRuntimeConflictClass->new();
$multi_obj3->alt2('alt2_runtime');
dies_ok { $multi_obj3->primary('primary_runtime') } 'Cannot set primary when alt2 is already set';
...
my $mixed_obj = MixedConflictClass->new( runtime_only => 'runtime_val', different => 'diff_val' );
is( $mixed_obj->runtime_only, 'runtime_val', 'runtime_only set during construction' );
...
dies_ok { $mixed_obj2->runtime_only('runtime_val') } 'runtime_conflicts_with prevents runtime setting';
